...
(function generalize() {
  ...
  if (titleEl) {
    ...
    titleEl.title = cfg.title;   // full title on hover
    titleEl.style.cssText += ';max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;';
  }
  ...
  if (pm) [...pm.options].forEach(o => {
    ...
    if (!ok) { o.remove(); return; }
    if (vlabels[v]) o.textContent = vlabels[v] + (is3d ? ' (3D)' : ' (2D)');  // relabel (e.g. PaCMAP slot → PCA)
  });
  ...
  // On-demand filters: a single "+ add filter" picker; each chosen attribute adds
  // one removable range row. Scales to any number of continuous attributes (vs a
  // box per attribute, which is unusable past a handful).
  function buildFilterRail() {
    ...
    bar.innerHTML = '<span style="color:var(--muted);font-size:11px;">filters:</span>'
      + `<select id="filter-add" title="add a numeric-range filter" style="font-size:11px;max-width:150px;">`
      + '<option value="">+ add filter…</option>'
      + cont.map(c => `<option value="${c.key}">${c.label}</option>`).join('') + '</select>'
      + '<span id="filter-active" style="display:inline-flex;gap:10px;flex-wrap:wrap;align-items:center;"></span>';
    const active = bar.querySelector('#filter-active');
    const byKey = Object.fromEntries(cont.map(c => [c.key, c]));
    function addRow(key) {
      const c = byKey[key];
      if (!c || active.querySelector(`[data-key="${key}"]`)) return;
      const g = document.createElement('span'); g.className = 'group'; g.dataset.key = key; g.style.gap = '3px';
      g.innerHTML = `<label style="color:var(--muted);font-size:10px">${c.label}</label>`
        + `<input type="number" class="f-min" step="any" placeholder="${(+c.range[0]).toPrecision(3)}" style="width:58px">`
        + `<span style="color:var(--muted)">–</span>`
        + `<input type="number" class="f-max" step="any" placeholder="${(+c.range[1]).toPrecision(3)}" style="width:58px">`
        + `<span class="rm" title="remove filter" style="cursor:pointer;color:var(--muted);padding:0 2px;">✕</span>`;
      ...
      g.querySelector('.rm').addEventListener('click', () => { delete filt[key]; g.remove(); computeFilterMatch(); });
    }
    bar.querySelector('#filter-add').addEventListener('change', e => {
  ...
  // ---- public API so an analysis panel can add computed results as color-by
  // attributes and recolor the cloud (used by sc1-py's analysis side-panel) -----
  const PAL = [[31,119,180],[255,127,14],[44,160,44],[214,39,40],[148,103,189],[140,86,75],
  ...
  function addOption(key, label) {
    const cb2 = document.getElementById('color-by');
    if (cb2 && !cb2.querySelector(`option[value="${CSS.escape(key)}"]`)) {
      const o = document.createElement('option'); o.value = key; o.textContent = label; cb2.appendChild(o);
    }
  }
  window.__atlas = {
    n: () => docs.length,
    ...
    config: () => cfg,
    addContinuous(key, label, vals, colormap = 'viridis') {
      const a = Float32Array.from(vals); let lo = Infinity, hi = -Infinity;
      for (const x of a) { if (x < lo) lo = x; if (x > hi) hi = x; }
      num[key] = a; spec[key] = { key, label, kind: 'continuous', source: 'num:' + key, colormap, range: [lo, hi] };
      addOption(key, label);
    },
    addCategorical(key, label, codes, vocab, palette) {
      cat[key] = Uint16Array.from(codes);
      spec[key] = { key, label, kind: 'categorical', source: 'cat:' + key, vocab,
        palette: palette || vocab.map((_, i) => PAL[i % PAL.length]) };
      addOption(key, label);
    },
    setColor(key) {
      colorMode = key; feat = null;
      const cb2 = document.getElementById('color-by'); if (cb2) cb2.value = key;
      buildLegend(); activeDataCache = null; rerender();
    },
    async colorFeature(name) {            // color by a lazily-fetched feature (gene)
      if (!cfg.feature_endpoint) return false;
      const r = await fetch(cfg.feature_endpoint.replace('{id}', encodeURIComponent(name)));
      if (!r.ok) return false;
      const v = new Float32Array(await r.arrayBuffer());
      let lo = Infinity, hi = -Infinity; for (const x of v) { if (x < lo) lo = x; if (x > hi) hi = x; }
      feat = { vals: v, lo, hi, label: name + ' (feature)' }; colorMode = '__feature';
      const cb2 = document.getElementById('color-by'); if (cb2) cb2.selectedIndex = -1;
      buildLegend(); activeDataCache = null; rerender(); return true;
    },
  };
  function panel() {
  ...
  function wireInteractive() {
    ...
    if (cfg.feature_endpoint && controls) {
      ...
      inp.placeholder = 'gene…'; inp.title = 'color by any gene/feature — type a name + Enter';
      inp.style.cssText = 'width:78px;margin-left:6px;background:#11141c;color:#cdd3e0;border:1px solid #2a2e3a;border-radius:4px;padding:2px 6px;';
    ...
    if (cfg.de_endpoint && controls) {
      ...
      btn.textContent = '⚲ DE'; btn.className = 'help-btn';
      btn.title = 'differential expression of the lassoed/searched selection vs the rest';
